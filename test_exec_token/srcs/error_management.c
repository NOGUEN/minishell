...
void			ft_print_unset_err(t_cmd_list *cmd_list)
{
	int i;
	...
	{
		if (ft_valid_key(cmd_list->tokens[i].cmd) == 0)
			error_write("minishell: %s: `%s': not a valid identifier\n", cmd_list->tokens[0].cmd, cmd_list->tokens[i].cmd);
		i++;
	}
}

void			ft_print_export_err(t_cmd_list *cmd_list)
{
	int i;
...
void			print_errstr(t_cmd_list *cmd_list)
{
    ...
    if (cmd_list->err_manage.errtoken)
		  ...
		  error_write("minishell: syntax error near unexpected token `%s\'\n", cmd_list->tokens[cmd_list->err_manage.errindex].cmd, 0);
		g_exit_status = 258;
	}
