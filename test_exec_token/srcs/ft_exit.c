...
int				check_digit(t_cmd_list *cmd_list)
{
	int			i;

	i = -1;
	if (cmd_list->tokens[1].cmd[0] == '-')
		i++;
	while (cmd_list->tokens[1].cmd[++i] != '\0') // 인자로 입력된 문자열에서 문자 하나씩 확인합니다.
		if (ft_isdigit(cmd_list->tokens[1].cmd[i]) == 0) // 문자가 숫자가 아닐경우, 0을 반환합니다.
		{
...
int				ft_exit(t_cmd_list *cmd_list)
{
	long long	result;
	int			err_flag;
	...
	if (cmd_list->exit_flag == 1) // exit 문자열은 파이프없이 단독으로 exit 명령어만 주어졌을 때 출력됩니다. 그리고 exit_flag 역시 파이프가 없을 떄 설정됩니다.
		printf("exit\n");
	if (cmd_list->tokens[1].cmd != NULL) // exit 명령어에 인자가 주어졌을 때
	{
		...
		if (cmd_list->tokens[2].cmd != NULL) // 인자가 2개 이상 입력되었을 때, 오류로 인식하여 -1을 반환합니다.
		{
	...
	{
		result = ft_atoi(cmd_list->tokens[1].cmd);//, &err_flag);
		if (err_flag == 1)
		{
			cmd_list->err_manage.errcode = 4;
			cmd_list->err_manage.errindex = 1;
